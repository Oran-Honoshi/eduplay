...
function renderMathVisualSVG(data: any, color = '#4A7FD4'): string {
  ...
  switch (data.type) {
    ...
    case 'angle': {
      ...
      return `<svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
        <line x1="${cx}" y1="${cy}" x2="${x2}" y2="${cy}" stroke="${stroke}" stroke-width="2" stroke-linecap="round"/>
        ...
        ${label(ax as any, ay as any, `${deg}°`, 12, 'bold', hl)}
      </svg>`
    }
    ...
    case 'rectangular_prism': {
      const sw = 80, sh = 50, depth = 28, ox = 50, oy = 60
...
function renderVisual(q: any, color = '#4A7FD4'): string {
  ...
  if (q.visual_data.type) {
    ...
    return svg ? `<div class="math-visual">${svg}</div>` : ''
  }
  ...
  const questionsHTML = questions.map((q: any, i: number) => {
    ...
    if (q.options) {
      answerHTML = `<div class="options-grid">${q.options.map((opt: any) => {
        ...
        return `<div class="option"><span class="opt-label">${opt.label}.</span><span class="opt-value">${renderValue(displayVal)}</span></div>`
      }).join('')}</div>`
    } else {
    ...
    return `
      <div class="question">
        <div class="q-left">
          ...
          <div class="q-body">${enText}${heText}${visualHTML}${answerHTML}${hintHTML}</div>
        </div>
        <div class="q-workspace">
          <div class="workspace-label">${isHebrew ? 'עבודה' : 'Working'}</div>
          <div class="workspace-lines">${Array.from({ length: 6 }, () => '<div class="ws-line"></div>').join('')}</div>
        </div>
      </div>`
  }).join('')
