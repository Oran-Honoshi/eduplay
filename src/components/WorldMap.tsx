...
// ── This file DOES exist in world-atlas@2 ────────────────────
const GEO_URL = 'https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json'

// Numeric country ID → continent slug
// (same mapping from your original WorldMap that was working)
const ID_TO_CONTINENT: Record<string, string> = {
  // Africa
  '012':'africa','024':'africa','072':'africa','108':'africa','120':'africa',
  '132':'africa','140':'africa','174':'africa','175':'africa',
  ...
  '504':'africa','508':'africa','516':'africa','562':'africa','566':'africa',
  '624':'africa','638':'africa','646':'africa','678':'africa',
  ...
  '096':'asia','104':'asia','116':'asia','142':'asia','144':'asia',
  '156':'asia','158':'asia','196':'asia','268':'asia','356':'asia',
  ...
  '512':'asia','524':'asia','586':'asia','608':'asia','634':'asia',
  '682':'asia','702':'asia','704':'asia','760':'asia','762':'asia',
  '764':'asia','784':'asia','792':'asia','800':'asia',
  '860':'asia','887':'asia','275':'asia',
  // Europe
  ...
  '470':'europe','492':'europe','498':'europe','499':'europe',
...
export default function WorldMap({ activePhase, completedPhases, onSelect }: WorldMapProps) {
  ...
  function getOpacity(slug: string | null, isHighlit: boolean): number {
    if (!slug) return 0.4
    if (isHighlit) return 1
    if (completedPhases.includes(slug)) return 0.88
    return 0.5
  }
          ...
          <Geographies geography={GEO_URL}>
            {({ geographies }: { geographies: any[] }) =>
              geographies.map((geo: any) => {
                ...
                const opacity = getOpacity(slug, isHighlit)
                const stroke  = isHighlit
                  ? CONTINENT_CONFIG[slug!]?.activeColor ?? '#fff'
                  : isComp
                    ? `${CONTINENT_CONFIG[slug!]?.color}80`
                    : '#061624'
                const sw = isHighlit ? 1.5 : isComp ? 0.6 : 0.3

                return (
                  <g key={geo.rsmKey}>
                    {/* Glow layer — blurred duplicate behind main shape */}
                    {isHighlit && (
                      <Geography
                        geography={geo}
                        fill={fill}
                    ...
                    {/* Main country shape (colored by continent) */}
                    <Geography
                      geography={geo}
                      fill={fill}
                      stroke={stroke}
          ...
          return (
            <button
              ...
              style={{
                ...
                color: isA ? r.active : 'rgba(255,255,255,0.65)',
